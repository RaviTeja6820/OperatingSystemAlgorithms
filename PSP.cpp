...
{
	public:
		int AT[10],BT[10],BTC[10],P[100],BQ[10],PQ[10],CT[10],RT[10],WT[10],TAT[10],Pr[10];
		float AWT,ATAT,ART;
		PSP()
		{
			...
			cout<<"Enter Priority of every process"<<endl;
			for(int i=0;i<np;i++){
				cin>>Pr[i];
			}
		...
		void reccur()
		{
			...
			for(t=0;t<n;t++)
			{
				...
				int index=search(PQ[min(PQ,count)],Pr,np);
				int ind=search(Pr[index],PQ,count);
				for(e=0;e<np;e++)
				{
					if(RQ[e]!=0)
					break;
